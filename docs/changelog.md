...
* [tmux_format_subscriptions](config/lua/config/tmux_format_subscriptions.md):
  ...
  tmux format strings (e.g. `#{T:status-left}`, `#{pane_current_path}`) and emits
  a [tmux-subscription-changed](config/lua/window-events/tmux-subscription-changed.md)
  event with each recomputed value, so they can be shown in the wezterm status bar.
