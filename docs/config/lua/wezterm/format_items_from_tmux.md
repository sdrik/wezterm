...
status bar when attached to a `tmux -CC` session. Feed it the `value` from a
[tmux-subscription-changed](../window-events/tmux-subscription-changed.md) event
and pass the result straight to `wezterm.format`:
...
wezterm.on('tmux-subscription-changed', function(window, pane, name, value, meta)
  if name == 'status_left' then
    window:set_left_status(wezterm.format(wezterm.format_items_from_tmux(value)))
  end
end)
...
```lua
local items = wezterm.format_items_from_tmux(value)
