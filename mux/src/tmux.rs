...
/// Build the `Action` for an OSC 1337 SetUserVar sequence, so the tmux
/// integration can set a pane user var by feeding it through the normal
/// `Pane::perform_actions` path — exactly as if the application running in the
/// pane had emitted the escape sequence itself.
pub(crate) fn set_user_var_action(name: String, value: String) -> Action {
    Action::OperatingSystemCommand(Box::new(OperatingSystemCommand::ITermProprietary(
        ...
        for event in events.iter() {
            ...
            match event {
                ...
                Event::SubscriptionChanged {
                    ...
                    // the remote_panes lock while calling perform_actions (which
                    ...
                    for local_pane_id in targets {
                        if let Some(p) = mux.get_pane(local_pane_id) {
                            p.perform_actions(vec![set_user_var_action(
                                name.clone(),
                                value.clone(),
                            )]);
                        }
